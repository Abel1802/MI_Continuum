...
class AE_Key(nn.Module):
    def __init__(self, emb_size=128):
        super(AE_Key, self).__init__()
        self.emb_size = emb_size
        self.lays = nn.Sequential(nn.Linear(1, 512),
                                  nn.ReLU(),
                                  nn.Linear(512, 512),
                                  nn.ReLU())
        self.lays2 = nn.Sequential(nn.Linear(512, 512),
                                  nn.ReLU(),
                                  nn.Linear(512, 1)
                                  )
    def forward(self, lf0):
        lf0 = lf0.unsqueeze(1)
        z = self.lays(lf0)
        lf0_pred = self.lays2(z)
        return lf0_pred

    def get_emb(self, lf0):
        lf0 = lf0.unsqueeze(1)
        return self.lays(lf0)
...
class DecoderMulti(nn.Module):
    def __init__(self, emb_size=1, c_in=64, c_out=80, dim_pre=512):
        ...
        # rnn layer
        self.lstm1 = nn.LSTM(c_in+emb_size*2, dim_pre, 1, batch_first=True)
        self.lstm2 = nn.LSTM(dim_pre, dim_pre*2, 1, batch_first=True)
        self.linear_projection = LinearNorm(dim_pre*2, c_out)
        # postnet
        self.postnet = Postnet()

    def forward(self, z, lF1_emb, lF2_emb):
        # concat
        x = torch.cat([z, lF1_emb, lF2_emb], dim=-1)
        x, _ = self.lstm1(x)
        x = x.permute(0, 2, 1)
        # conv layer
...
if __name__ == "__main__":
    ...
    _, outs = decoder(z, lf0_emb, lf0_emb) 
